...
{  
	...
	{   
		if (null == p)    throw new ExceptionPositionInvalid("意外：传递给List_DLNode的位置是null");   
		if (header == p)  throw new ExceptionPositionInvalid("意外：头节点哨兵位置非法");  
		if (trailer == p) throw new ExceptionPositionInvalid("意外：尾结点哨兵位置非法"); 
	...
	{   
		if (isEmpty())    throw new ExceptionListEmpty("意外：列表空");
	...
	//返回紧靠给定位置之前的元素（的位置） 
	 ...
	 {  
		 ...
		 if (prev == header)    throw new ExceptionBoundaryViolation("意外：企图越过列表前端"); 
		 return prev; 
	 }  
	//返回紧接给定位置之后的元素（的位置）
	 ...
	 {   
		 ...
		 if (next == trailer)    throw new ExceptionBoundaryViolation("意外：企图越过列表后端"); 
		 return next;  
	}  
	//将e插入至紧靠给定位置之前的位置
	...
	//删除给定位置处的元素，并返回之 
	 ...
	 {   
		 ...
		 vPrev.setNext(vNext);
		 vNext.setPrev(vPrev);
		 Object vElem = v.getElem();  //将该位置（节点）从列表中摘出，以便系统回收其占用的空间 
		 v.setNext(null); 
		 v.setPrev(null); 
		 return vElem;    
	 }
